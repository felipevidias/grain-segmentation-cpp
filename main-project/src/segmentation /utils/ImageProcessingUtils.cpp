 ...
 */
 ...
 std::vector<Centroid> calculate_centroids(const xt::xtensor<uint32_t, 3>& labeled_image) {
     // Using a map to store sums and counts for each label
     ...
     for (const auto& pair : sums) {
         ...
         if (count > 0) {
             // Centroid coordinates are double for precision
             centroids.push_back({
                 coords[0] / count,
                 coords[1] / count,
                 coords[2] / count,
 ...
 void positioning_label(xt::xtensor<uint32_t, 3>& labeled_image, const xt::xtensor<uint32_t, 3>& labeled_crop,
     ...
     // Calculate the top-left corner of the paste destination in the large image
